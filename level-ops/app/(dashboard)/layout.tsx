...
export default function DashboardLayout({
  ...
  return (
    <div className="flex min-h-screen bg-white">
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 py-2.5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              ...
              <div className="hidden md:flex items-center gap-1">
                ...
                >
                  <Search className="h-[25px] w-[25px]" aria-hidden="true" />
                </Link>
              </div>

              {/* Mobile & Desktop: AI Assistant button */}
              <button
                ...
                title="AI Assistant"
              >
                <Bot className="h-[25px] w-[25px]" aria-hidden="true" />
              </button>

              {/* Hamburger menu (includes mobile-specific items) */}
              <UserMenu />
            </div>
          </div>
        </header>

        <main className="flex-1 mx-auto max-w-7xl px-4 sm:px-6 py-6 w-full">
          <ErrorBoundary>
            {children}
          </ErrorBoundary>
        </main>
      ...
      {/* Fixed sidebar on the right - chat is always mounted, just hidden */}
      <div
        className={`fixed top-0 right-0 bottom-0 w-96 bg-white border-l border-gray-200 transition-transform duration-300 ease-in-out z-50 flex flex-col ${
